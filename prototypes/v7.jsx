...
/* v7 — ajout du registre de réception (apport Chapman, cf. docs/POSITIONNEMENT.md)
   Reprend v6 : interface neutre, alternatives par mot, "message adapté tel quel",
   six techniques, garde-fous coercition/danger. */
...
/* Registres de réception — langage courant, jamais le vocabulaire d'origine */
const REGISTRES = [
  { id: null, court: "Indifférent", aide: "Aucun réglage particulier" },
  { id: "reconnaissance", court: "Reconnaissance", aide: "A besoin qu'on nomme ce qu'il ou elle fait" },
  { id: "concret", court: "Du concret", aide: "Attend une action, pas des mots" },
  { id: "temps", court: "Du temps", aide: "A besoin d'un moment ensemble" },
  { id: "lien", court: "De la réassurance", aide: "A besoin de savoir que le lien tient" },
  { id: "marge", court: "De la marge", aide: "A besoin qu'on ne l'enferme pas" },
];

const REG_CONSIGNE = {
  reconnaissance: "Le destinataire reçoit surtout ce qui nomme et reconnaît ce qu'il fait, fournit ou porte. Mets cela en avant sans flatterie.",
  concret: "Le destinataire reçoit surtout le concret. Propose une action, un geste ou une solution précise plutôt que des considérations.",
  temps: "Le destinataire reçoit surtout le temps partagé. Propose un moment pour en parler ensemble.",
  lien: "Le destinataire a besoin d'être rassuré sur la solidité de la relation. Rappelle explicitement que le lien n'est pas remis en cause.",
  marge: "Le destinataire a besoin d'autonomie. Laisse-lui une marge, ne l'enferme pas dans une réponse obligée, évite l'injonction.",
};

const TECHNIQUES = `TECHNIQUES DE REFORMULATION (à appliquer, jamais à nommer) :
1. Fait précis plutôt que jugement.
2. Parler de soi plutôt que de l'autre : retirer l'accusation sans retirer le fond.
3. Demande claire et négociable à laquelle on peut dire oui.
4. Ouvrir sur une exception ou une issue concrète quand c'est pertinent.
5. Supprimer "jamais"/"toujours" au profit d'une période précise.
6. S'en prendre au problème, pas à la personne.

INTERDICTION LEXICALE ABSOLUE dans tes sorties : coaching, coach, médiation, médiateur, CNV, communication non violente, bienveillance, thérapie, développement personnel, "langage de l'amour", "besoin profond", "travail sur soi", "émotion". Tu es un outil d'écriture, pas un accompagnement.`;

const GARDE_FOUS = `GARDE-FOUS PRIORITAIRES (priment sur tout) :
- COERCITION : menace, chantage, pression, humiliation, contrôle d'une personne → tu ne reformules pas. Renvoie {"blocage":"coercition"}.
...
MESSAGE DÉJÀ ADAPTÉ : si le message est proportionné et ne dessert pas son auteur — y compris s'il est ferme, direct ou en net désaccord — tu ne l'adoucis PAS. Une personne a le droit d'être ferme. Renvoie "adapte":true avec une "note" expliquant pourquoi il fonctionne tel quel. N'invente pas de retouche pour justifier ton existence.

Sinon "adapte":false et :
- "proche" : intervention MINIMALE, en segments consécutifs qui concaténés reforment EXACTEMENT le texte retouché. Sur chaque segment modifié ("c":true), fournis "alt" : 2 variantes d'intensité différente pour ce seul passage, insérables grammaticalement à sa place. Segments inchangés : "c":false, pas de "alt".
- "ouvert" : version qui ouvre le dialogue.
- "limite" : version qui pose clairement une limite, ferme et respectueuse.
- "note" : UNE phrase factuelle sur l'ajustement fait. Concret, jamais moralisateur.
...
- "lecture" : 2 phrases maximum sur ce que l'expéditeur cherche peut-être à dire, y compris la possibilité qu'il s'exprime simplement dans un registre différent. En HYPOTHÈSE ("il se peut que"), jamais en diagnostic, sans vocabulaire psychologisant.
...
export default function App() {
  ...
  const [segs, setSegs] = useState([]); const [open, setOpen] = useState(null); const [plus, setPlus] = useState(null);
  ...
  useEffect(() => {
    const t = rawE.trim(); const k = t + "|" + (registre || "");
    if (t.length < 12 || k === keyE.current) return;
    const timer = setTimeout(async () => {
      keyE.current = k; const id = ++idE.current; setStE("loading");
      const reg = registre ? `\n\nREGISTRE DE RÉCEPTION DU DESTINATAIRE : ${REG_CONSIGNE[registre]} Applique-le aux trois versions, sans jamais l'expliciter dans le message.` : "";
      try {
        const p = await callAPI(SP_ECRIRE, `"""${t}"""${reg}`);
  ...
  function choisir(i, texte) { setSegs((p) => p.map((s, k) => (k === i ? { ...s, t: texte } : s))); setOpen(null); setCopied(""); }
  ...
  const regActif = REGISTRES.find((r) => r.id === registre);
      ...
      <div style={{ maxWidth: 460, margin: "0 auto", padding: "24px 20px 60px" }}>
        <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <Marque />
            <div>
              <h1 style={{ fontFamily: "'Fraunces', ui-serif, Georgia, serif", fontWeight: 500, fontSize: 24, letterSpacing: "-0.01em", margin: 0 }}>Nuance</h1>
              <p style={{ margin: "1px 0 0", fontSize: 12, color: C.soft }}>Dire les choses avec justesse.</p>
            </div>
          </div>
          <button onClick={() => setDetail((v) => !v)} aria-pressed={detail}
            style={{ border: `1px solid ${detail ? C.accent : C.line}`, background: detail ? C.accentSoft : "transparent", color: detail ? C.accent : C.soft, borderRadius: 8, padding: "6px 11px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Détails</button>
        </header>
        ...
        <div style={{ display: "flex", borderBottom: `1px solid ${C.line}`, marginBottom: 18 }}>
          {[{ id: "ecrire", label: "J'écris" }, { id: "recevoir", label: "Je reçois" }].map((m) => {
            ...
            return <button key={m.id} onClick={() => setMode(m.id)}
              style={{ flex: 1, border: "none", background: "none", padding: "10px 0 11px", color: on ? C.ink : C.faint, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", borderBottom: on ? `2px solid ${C.accent}` : "2px solid transparent", marginBottom: -1 }}>{m.label}</button>;
          })}
        </div>
        ...
        {mode === "ecrire" && (
          <>
            ...
            {/* Registre de réception — facultatif */}
            <div style={{ marginTop: 12 }}>
              <div style={{ fontSize: 12, color: C.faint, marginBottom: 7 }}>La personne à qui vous écrivez est sensible à…</div>
              <div className="chips" style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 2 }}>
                {REGISTRES.map((r) => {
                  const on = registre === r.id;
                  return <button key={r.court} onClick={() => setRegistre(r.id)} title={r.aide}
                    style={{ flexShrink: 0, border: `1px solid ${on ? C.accent : C.line}`, background: on ? C.accentSoft : C.surface, color: on ? C.accent : C.soft, borderRadius: 999, padding: "7px 12px", fontSize: 12.5, fontWeight: on ? 600 : 500, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}>{r.court}</button>;
                })}
              </div>
              {registre && <div style={{ fontSize: 11.5, color: C.faint, marginTop: 6 }}>{regActif.aide}</div>}
            </div>
            ...
            : resE?.adapte ? (
              <div className="rise" style={{ background: C.surface, border: `1px solid ${C.accentLine}`, borderLeft: `3px solid ${C.accent}`, borderRadius: 10, padding: "16px", animation: "rise .35s ease both" }}>
                ...
                <button onClick={() => copy(rawE.trim(), "adapte")}
                  ...
                  {copied === "adapte" ? "Copié ✓" : "Copier mon message"}</button>
              </div>
            ) : resE ? (
              <div className="rise" style={{ animation: "rise .35s ease both" }}>
                <div style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: 10, padding: "16px" }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: C.soft, marginBottom: 10 }}>
                    ...
                    {registre && !plus && <span style={{ color: C.faint, fontWeight: 400 }}> · {regActif.court.toLowerCase()}</span>}
                  </div>
                  <p style={{ margin: 0, fontFamily: "'Fraunces', ui-serif, Georgia, serif", fontSize: 17.5, lineHeight: 1.65 }}>
                    {plus ? resE[plus] : segs.map((s, i) => s.c ? (
                      <button key={i} onClick={() => setOpen(open === i ? null : i)}
                        style={{ border: "none", background: open === i ? C.accent : C.accentSoft, color: open === i ? "#fff" : C.accent, font: "inherit", padding: "1px 3px", borderRadius: 4, cursor: "pointer", borderBottom: open === i ? "none" : `1.5px solid ${C.accentLine}` }}>{s.t}</button>
                    ) : <span key={i}>{s.t}</span>)}
                  </p>
                    ...
                    <div className="rise" style={{ marginTop: 12, background: C.bg, border: `1px solid ${C.line}`, borderRadius: 8, padding: "10px", animation: "rise .2s ease both" }}>
                      ...
                      {[segs[open].orig, ...segs[open].alt].map((a, k) => (
                        <button key={k} onClick={() => choisir(open, a)}
                          style={{ display: "block", width: "100%", textAlign: "left", border: `1px solid ${a === segs[open].t ? C.accent : C.line}`, background: a === segs[open].t ? C.accentSoft : C.surface, color: C.ink, borderRadius: 6, padding: "9px 11px", fontSize: 14.5, marginBottom: 6, cursor: "pointer", fontFamily: "'Fraunces', ui-serif, Georgia, serif", lineHeight: 1.4 }}>{a}</button>
                      ))}
                    </div>
                  )}
                  ...
                  <button onClick={() => copy(texteFinal, "e")}
                    style={{ width: "100%", marginTop: 14, border: "none", borderRadius: 8, background: C.accent, color: "#fff", padding: "12px", fontSize: 14.5, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>{copied === "e" ? "Copié ✓" : "Copier"}</button>
                </div>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                  {[{ id: "ouvert", label: "Ouvrir davantage" }, { id: "limite", label: "Poser une limite" }].map((v) => {
                    ...
                    return <button key={v.id} onClick={() => { setPlus(on ? null : v.id); setCopied(""); setOpen(null); }}
                      style={{ flex: 1, border: `1px solid ${on ? C.accent : C.line}`, borderRadius: 8, background: on ? C.accentSoft : "transparent", color: on ? C.accent : C.soft, padding: "10px 6px", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>{v.label}</button>;
                  })}
                </div>
                {plus && <button onClick={() => { setPlus(null); setCopied(""); }}
                  style={{ width: "100%", marginTop: 8, border: "none", background: "none", color: C.faint, fontSize: 12.5, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", padding: "6px" }}>← Revenir à la proposition</button>}
              </div>
              ...
              <div className="rise" style={{ animation: "rise .35s ease both" }}>
                <div style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: 10, padding: "16px" }}>
                  ...
                  <button onClick={() => copy(resR[ton], "r")}
                    style={{ width: "100%", marginTop: 14, border: "none", borderRadius: 8, background: C.accent, color: "#fff", padding: "12px", fontSize: 14.5, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>{copied === "r" ? "Copié ✓" : "Copier"}</button>
                </div>
                <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                ...
                <button onClick={() => setVoirLecture((v) => !v)}
                  ...
                  {voirLecture ? "Masquer l'autre côté" : "L'autre côté →"}</button>
                {voirLecture && (
...
/* Symbole : deux cercles qui se recouvrent */
function Marque() {
  return (
    <svg width="34" height="34" viewBox="0 0 100 100" aria-hidden="true">
      <rect width="100" height="100" rx="23" fill="#FAFAF9" stroke="#DDE3E1" />
  ...
  return (
    <div style={{ background: C.warnSoft, border: `1px solid #E5CDC5`, borderLeft: `3px solid ${C.warn}`, borderRadius: 10, padding: "16px" }}>
      <p style={{ margin: 0, fontSize: 14.5, lineHeight: 1.55, fontWeight: 600, color: C.warn }}>
        {danger ? "Ce message évoque une situation qui dépasse une question de formulation." : "Nuance ne reformule pas ce type de message."}</p>
      <p style={{ margin: "8px 0 0", fontSize: 14, lineHeight: 1.55, color: C.soft }}>
        {danger ? "Des personnes sont là pour en parler, gratuitement et à toute heure : 3919 (violences), 3114 (souffrance psychique), 17 en cas d'urgence."
                : "Rendre plus présentable un message qui fait pression sur quelqu'un ne réglerait rien."}</p>
    </div>
  );
}
function Vide({ text }) {
